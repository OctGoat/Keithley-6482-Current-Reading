...
parser = argparse.ArgumentParser(
    description='Welcome to Draculog v.' + version_str + ' help menu. Draculog is a simple software data logger for Keithley 6482 pico-ammeter.')
    ...
    while True:
        ...
        try:
            ...
            if args.nosave == False and i % BACK_UP_AFTER == 0:
                print('Backing-up to ' + 'draculog' + time_str + alphaid + '_backup.dat...')
                arr_backup = arr
                if j == 0:
                    save_to_(arr_backup, 'draculog' + time_str + alphaid + '_backup.dat')
                else:
                    save_to_(arr_backup[BACK_UP_AFTER * j:BACK_UP_AFTER * (j + 1)],
                             'draculog' + time_str + alphaid + '_backup.dat', append=True)
                j += 1
            i += 1
            # num = 0
            ...
            if not args.nosave: #args.nosave == False
                print('All done, saving to ' + 'draculog' + time_str + alphaid + '.dat...')
                save_to_(arr, 'draculog' + time_str + alphaid + '.dat')

            ser.write(b'ABORt\r\n')
            ser.close()
            print('Goodbye!')
            # If backup file exists, delete it
            if os.path.isfile('draculog' + time_str + alphaid + '_backup.dat'):
                os.remove('draculog' + time_str + alphaid + '_backup.dat')
            else:
                print("Warning: Backup file not found.")
            # print(os.path.isfile('draculog' + time_str + alphaid + '_backup.dat'))  #False
            sys.exit(0)
